(function() {
	...
	const {Observable, BehaviorSubject} = require("observable");
				...
				return new Observable(observer => {
					...
					for (const token of tokens) {
						...
						token.watch = (object, prop) => {
							...
							watchers.push({object, prop, token});
						}
					}
					...
					const nextValue = () => {
						...
						subscription = Observable.from(watchers)
							.mergeMap(({object, prop, token}) => {

								// if (object[prop] instanceof Observable) {
								// 	token.value = object[prop];
								// 	token.subject = new BehaviorSubject();
								// 	token.value.subscribe(token.subject);
								//
								// 	window.s = token.subject;
								//
								// 	return token.subject;
								// }

								return watch$$(object, prop, token);
							})

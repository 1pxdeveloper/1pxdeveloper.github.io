(function() {
	...
	class JSContext {
		...
		/// @TODO: thisObj
		static connect(thisObj, globalObj, ...locals) {
			let context = new JSContext(thisObj, globalObj, ...locals);
		...
		constructor(thisObj, globalObj, ...locals) {
		...
		fork(local) {
			...
			return JSContext.connect(this.thisObj, this.globalObj, ...[local].concat(this.locals));
		}

		evaluate(script) {
			return $parse(script).evaluate(this.thisObj, this.globalObj, ...this.locals);
		}

		assign(script, value) {
			return $parse(script).assign(value, this.thisObj, this.globalObj, ...this.locals);
		}
		...
		watch$(script, callback) {
			...
			let script$ = $parse(script).watch(this.thisObj, this.globalObj, ...this.locals).takeUntil(this.disconnect$);

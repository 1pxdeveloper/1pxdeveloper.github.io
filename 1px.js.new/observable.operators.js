(function() {
	...
	const reject = (callback) => filter((...args) => !callback(...args));
	...
	const delayWhen = (delayDurationSelector) => (observable) =>
		observable.concatMap((value, index) => delayDurationSelector(value, index).take(1).mapTo(value));
